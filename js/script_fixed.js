...
document.addEventListener('DOMContentLoaded', () => {
    ...
    const createProductCard = (product, isFavorite = false) => {
        ...
        productCard.innerHTML = `
            ...
            <div class="product-card-actions">
                ...
                <button class="compare-btn" data-name="${product.name}" title="Comparar preços semelhantes">
            ...
            if (compareBtn) {
                compareBtn.onclick = () => {
                    const compareName = (
                        ...
                        ((document.getElementById('detail-name')?.textContent || '').trim())
                    );
                    if (!compareName) return;

                    // Fecha o modal de produto primeiro para evitar sobreposição/conflito visual
                    try {
                        if (typeof pjCloseModal === 'function') pjCloseModal();
                    } catch (_) {}

                    const openCompareFlow = () => {
                        const openCompare = (typeof window.openCompareModal === 'function')
                            ? window.openCompareModal
                            : (typeof openCompareModal === 'function' ? openCompareModal : null);

                        if (openCompare) {
                            try {
                                openCompare(compareName);
                                return;
                            } catch (err) {
                                console.error('Falha ao abrir comparação pelo handler principal:', err);
                            }
                        }

                        // Fallback robusto: monta uma lista simples de comparação e abre o modal
                        try {
                            ...
                            const allProducts = (typeof getFromLocalStorage === 'function')
                                ? (getFromLocalStorage('products') || [])
                                : [];
                            const normalizedQuery = normalize(compareName);
                            const matches = allProducts
                                .filter((item) => {
                                    const itemName = normalize(item && item.name);
                                    if (!itemName || !normalizedQuery) return false;
                                    return itemName === normalizedQuery || itemName.includes(normalizedQuery) || normalizedQuery.includes(itemName);
                                })
                                .filter((item, index, arr) => index === arr.findIndex((x) => String(x.id) === String(item.id)))
                                .slice(0, 20);

                            if (compareTitle) compareTitle.textContent = `Comparar: ${compareName}`;
                            if (compareList) compareList.innerHTML = '';

                            if (matches.length > 0 && compareList) {
                                ...
                                matches.forEach((item) => {
                                    ...
                                    titleEl.className = 'compare-name';
                                    titleEl.textContent = item.name || 'Produto sem nome';

                                    const priceEl = document.createElement('div');
                                    priceEl.className = 'compare-price';
                                    const n = parseFloat(String(item.price ?? '').replace(',', '.')) || 0;
                                    priceEl.textContent = `€${n.toFixed(2).replace('.', ',')}`;

                                    const metaEl = document.createElement('div');
                                    metaEl.className = 'compare-meta';
                                    metaEl.textContent = `${item.market || 'Mercado não informado'}${item.brand ? ` • ${item.brand}` : ''}`;
                            ...
                            compareModal.classList.add('show', 'visible');
                            document.body.classList.add('modal-open');
                        } catch (_) {}
                    };

                    // Aguarda animação de fechamento do modal de produto para evitar competição de foco/camadas
                    setTimeout(openCompareFlow, 220);
                };
            }
        } catch (_) {}
    };
    ...
    // (legado) versão antiga mantida para referência
    const openCompareModal_legacy = (productName) => {
        const products = getFromLocalStorage('products');
        if (!products || products.length === 0) {
            console.warn("Nenhum produto encontrado para comparação.");
            return;
        }

        const normalizedQuery = normalizeName(productName);
        
        // Encontrar o produto base para ter uma categoria de referência
        const baseProduct = products.find(p => normalizeName(p.name) === normalizedQuery);
        const baseCategory = baseProduct ? baseProduct.category : null;

        // Filtrar produtos similares
        let similarProducts = products.filter(p => {
            const normalizedName = normalizeName(p.name);
            if (normalizedName === normalizedQuery) {
                return true; // Mesmo nome
            }
            if (baseCategory && p.category === baseCategory) {
                return true; // Mesma categoria
            }
            // Lógica de similaridade de nome mais simples
            return normalizedName.includes(normalizedQuery) || normalizedQuery.includes(normalizedName);
        });

        // Remover duplicados
        similarProducts = similarProducts.filter((p, index, self) => 
        ...
        if (similarProducts.length === 0) {
            ...
            const cheapestProduct = similarProducts.reduce((min, p) => (p.price < min.price) ? p : min, similarProducts[0]);
            
            compareList.innerHTML = '';
            similarProducts.forEach(p => {
                const priceDiff = p.price - cheapestProduct.price;
                const diffPercentage = (cheapestProduct.price > 0) ? ((priceDiff / cheapestProduct.price) * 100) : 0;

                let diffClass = '';
                let diffText = '';
                if (priceDiff > 0) {
                    diffClass = 'negative'; // Mais caro é "negativo" para o consumidor
                    diffText = `+${formatPrice(priceDiff)} (+${diffPercentage.toFixed(0)}%)`;
                } else if (priceDiff < 0) {
                    diffClass = 'positive';
                    diffText = `${formatPrice(priceDiff)} (${diffPercentage.toFixed(0)}%)`;
                }

                const isFavorite = getFromSessionStorage('favorites').some(fav => fav.id === p.id);

                const productCard = document.createElement('div');
                productCard.className = 'compare-item';
                productCard.dataset.productId = p.id;
                productCard.innerHTML = `
                    <div class="product-img-container">
                        <img src="${p.imageUrl || DEFAULT_IMAGE_URL}" alt="${p.name}">
                    </div>
                    <div class="product-info">
                        <div class="product-name">${p.name}</div>
                        <div class="product-market">${p.market}</div>
                        <div class="product-price-container">
                            <div class="product-price">${formatPrice(p.price)}</div>
                            ${diffText ? `<div class="price-diff ${diffClass}">${diffText}</div>` : ''}
                        </div>
                    </div>
                    <div class="compare-actions">
                        <button class="compare-action-btn add-to-cart-btn" data-id="${p.id}"><i class="fas fa-shopping-cart"></i></button>
                        <button class="compare-action-btn favorite-btn ${isFavorite ? 'favorited' : ''}" data-id="${p.id}"><i class="fas fa-heart"></i></button>
                    </div>
                `;
                
                // Evento para ver detalhes do produto
                productCard.addEventListener('click', (e) => {
                ...
                compareList.appendChild(productCard);
            });
        }
    ...
    // Tornar disponível globalmente para uso em event handlers
        ...
        comparePage._onPageClick = (e) => {
            ...
            if (btn) {
                ...
                if (btn.classList.contains('fav-btn')) {
                    ...
                    try { showToast(nowFav ? 'Adicionado aos favoritos' : 'Removido aos favoritos', { type: 'info' }); } catch(_) {}
                    return;
                }
                // Carrinho
    ...
    // Clique explícito no botão comparar (robusto contra conflitos de outros handlers)
    productsList.addEventListener('click', (e) => {
        ...
        const nameFromBtn = (compareBtn.dataset && compareBtn.dataset.name) ? compareBtn.dataset.name : '';
        const nameFromCard = card ? ((card.querySelector('.product-name')?.textContent || '').trim()) : '';
        const compareName = nameFromBtn || nameFromCard;
        if (!compareName) return;
        const openCompare = (typeof openCompareModal === 'function')
            ? openCompareModal
            : (typeof window.openCompareModal === 'function' ? window.openCompareModal : null);
        if (openCompare) {
            try { openCompare(compareName); return; }
            catch (err) { console.error('Falha ao abrir comparação:', err); }
        }
        const modalFallback = document.getElementById('compare-modal');
        const titleFallback = document.getElementById('compare-modal-title');
        if (titleFallback) titleFallback.textContent = `Comparar: ${compareName}`;
        if (modalFallback) {
    ...
    document.addEventListener('click', (e) => {
        ...
        // Fallback robusto: comparar por nome mesmo sem data-id
        if (targetBtn.classList.contains('compare-btn')) {
            const card = targetBtn.closest('.product-card');
            const nameFromBtn = (targetBtn.dataset && targetBtn.dataset.name) ? targetBtn.dataset.name : '';
            const nameFromCard = card ? ((card.querySelector('.product-name')?.textContent || '').trim()) : '';
            const compareName = nameFromBtn || nameFromCard;
            if (!compareName) return;
            const openCompare = (typeof openCompareModal === 'function')
...
// (bloco legado duplicado removido)
    ...
    const openCompareModal = (productName) => {
        ...
        const toPrice = (price) => parseFloat(String(price ?? '').replace(',', '.')) || 0;
        const buildComparableTokens = (name, brand = '') => {
            let base = normalizeName(name);
            const b = normalizeName(brand);
            if (b) {
                try {
                    base = base.replace(new RegExp(`\\b${escapeRegex(b)}\\b`, 'g'), ' ');
                } catch (_) { /* ignore */ }
            }
            base = base
                .replace(/\b\d+[a-z]{0,3}\b/g, ' ')
                .replace(/\b(kg|g|gr|gramas|ml|l|lt|cl|un|und|unidade|unidades|pacote|pack|pct|caixa|garrafa|lata|saco|frasco|refil)\b/g, ' ')
                .replace(/\b(de|da|do|das|dos|com|sem|tipo|extra|premium|tradicional|original|sabor|marca)\b/g, ' ')
                .replace(/\s+/g, ' ')
                .trim();
            return base.split(' ').filter(t => t && t.length > 2);
        };

        const requestedTokens = buildComparableTokens(productName, guessedBrand);
        const requestedKey = requestedTokens.join(' ');
        const requestedSet = new Set(requestedTokens);

        const getSimilarityScore = (candidateName, candidateBrand) => {
            const cTokens = buildComparableTokens(candidateName, candidateBrand);
            if (!cTokens.length || !requestedTokens.length) return 0;
            const common = cTokens.filter(t => requestedSet.has(t)).length;
            const reqCoverage = common / requestedTokens.length;
            const candCoverage = common / cTokens.length;
            const startsEqual = requestedTokens.slice(0, 2).join(' ') === cTokens.slice(0, 2).join(' ');
            if (startsEqual && common >= 2) return 1;
            return (reqCoverage * 0.65) + (candCoverage * 0.35);
        };

        let matches = products.filter((p) => {
            const nName = normalizeName(p.name);
            const cTokens = buildComparableTokens(p.name, p.brand);
            const cKey = cTokens.join(' ');
            if (!nName) return false;
            if (normalized && (nName === normalized || nName.includes(normalized) || normalized.includes(nName))) return true;
            if (requestedKey && cKey && cKey === requestedKey) return true;
            const score = getSimilarityScore(p.name, p.brand);
            return score >= 0.62;
        });

        if (matches.length < 2 && requestedTokens.length) {
            matches = products.filter((p) => {
                const cTokens = buildComparableTokens(p.name, p.brand);
                if (!cTokens.length) return false;
                const common = cTokens.filter(t => requestedSet.has(t)).length;
                return common >= Math.max(1, Math.min(2, requestedTokens.length));
            });
        }

        matches = matches.filter((item, index, arr) =>
    ...
    const cheapestGlobal = matches.length ? matches.reduce((min, it) => toPrice(it.price) < toPrice(min.price) ? it : min, matches[0]) : null;
        const cheapestBranded = branded.length ? branded.reduce((min, it) => toPrice(it.price) < toPrice(min.price) ? it : min, branded[0]) : null;
        const cheapestPrivate = privateLabel.length ? privateLabel.reduce((min, it) => toPrice(it.price) < toPrice(min.price) ? it : min, privateLabel[0]) : null;
    ...
    matches.sort((a,b) => toPrice(a.price) - toPrice(b.price));
    ...
    // Modo desktop redesenhado: produto mais barato em destaque no topo + lista compacta
        ...
        const highestPrice = matches.reduce((max, item) => toPrice(item.price) > toPrice(max.price) ? item : max, matches[0]);
        const savings = Math.max(0, toPrice(highestPrice.price) - toPrice(cheapestGlobal.price));
        const marketCount = new Set(matches.map(m => normalizeName(m.market || ''))).size;
        const summaryPanel = document.createElement('div');
        summaryPanel.className = 'compare-summary-panel';
        summaryPanel.innerHTML = `
            <div class="summary-pill"><strong>${matches.length}</strong><span>ofertas</span></div>
            <div class="summary-pill"><strong>${marketCount}</strong><span>mercados</span></div>
            <div class="summary-pill summary-pill-success"><strong>${formatPrice(savings)}</strong><span>economia máxima</span></div>
        `;
        desktopLayout.appendChild(summaryPanel);
        ...
        const diffPercent = (price) => {
            const base = toPrice(cheapestGlobal.price);
            const current = toPrice(price);
