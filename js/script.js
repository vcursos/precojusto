...
// Fun��o para popular os filtros (chama updateFilterOptions quando dispon�vel)
...
// Fun��o para carregar hist�rico de pesquisa
function loadSearchHistory() {
    console.log('?? Carregando hist�rico de pesquisa...');
    setTimeout(() => {
...
// Fun��o para atualizar badge do carrinho
...
document.addEventListener('DOMContentLoaded', () => { console.log('DOMConteudo iniciado');
    ...
    // Para dados que devem persistir (produtos, sugest�es)
    ...
    // Fun��o para gerir a visibilidade dos bot�es de scroll
    ...
    // Cria��o do card de produto para a exibi��o principal
    const createProductCard = (product, isFavorite = false) => {
        ...
        productCard.innerHTML = `
            ...
            <div class="product-card-actions">
                ...
                <button class="suggest-price-btn" data-id="${product.id}" title="Sugerir um novo pre�o">
    ...
    console.log('?? DEBUG - Campos:', { unit: p.unit, quantity: p.quantity, unitDisplay: p.unitDisplay, country: p.country, barcode: p.barcode });
        ...
        // Mostrar/ocultar se��o de descri��o
    ...
    // Cria��o do item de lista para os modais (carrinho e favoritos)
    ...
    // Renderiza��o dos Produtos na tela principal
    const renderProducts = (productsToRender = getFromLocalStorage('products')) => {
        ...
        // Garante que o estado dos bot�es � verificado ap�s a renderiza��o
    ...
    // Ordena por pre�o ascendente (mant�m refer�ncia do mais barato j� calculado)
    ...
    // Modo desktop redesenhado: produto mais barato em destaque no topo + lista compacta
        ...
        // Fun��o para calcular diferen�a percentual em rela��o ao mais barato
    ...
    rightCol.classList.add('branded');
        ...
        limitedBranded.forEach((it, index) => {
            ...
            if (index === 3 && limitedBranded.length > 3) {
                ...
                separator.innerHTML = '<hr><span>Mais op��es de marca</span><hr>';
    ...
    const renderRecentProducts = () => {
        ...
        if (!recentProductsList) return; // Prote��o caso o elemento n�o exista
    ...
    // Fun��o espec�fica para pesquisa por c�digo de barras
    ...
    // Ao pressionar Enter no campo de pesquisa, salvar no hist�rico e mostrar se��o
    ...
    // Ao clicar em um card de produto (fora dos bot�es), abrir modal de detalhe
    ...
    // Bot�o de pesquisar (se for adicionado futuramente): caso exista, faz mesma a��o do Enter
    ...
    document.addEventListener('click', (e) => {
        ...
        if (targetBtn.classList.contains('suggest-price-btn')) {
            ...
            modalSuggestionMarket.value = product.market || '';
                // zona removida: apenas pre�o edit�vel conforme solicitado
            // Apenas o campo de pre�o fica edit�vel - os demais permanecem disabled no HTML
    ...
    favoritesList.addEventListener('click', (e) => {
        ...
        saveToSessionStorage('favorites', favorites);
        renderFavorites();
        renderProducts(); // Atualiza a tela principal para desmarcar o cora��o
    });
    ...
    const suggestedPrice = modalSuggestionNewPrice.value;
        ...
        if (!suggestedPrice) {
            alert('Por favor, insira um pre�o v�lido.');
            return;
        }
        ...
        const suggestions = getFromLocalStorage('suggestions'); // Sugest�es persistem
        suggestions.push(newSuggestion);
        saveToLocalStorage('suggestions', suggestions); // Salva no localStorage

    alert('Sua sugest�o foi enviada com sucesso e ser� analisada pela administra��o!');
    closeModal(suggestionModal);
        modalSuggestionForm.reset();
    });

    // Fun��es para bloquear/desbloquear scroll da p�gina
    ...
    // Abrir modais
    // Helpers para abrir/fechar com anima��o
    ...
    const closeModal = (modal) => { console.log('?? closeModal chamada para:', modal);
        ...
        // aguarda anima��o antes de esconder e desbloquear scroll
        setTimeout(() => {
    ...
    window.addEventListener('click', (e) => {
        try {
            ...
            // seguran�a contra ambientes inesperados
            console.error('Erro ao avaliar clique no backdrop do modal:', err);
        }
    });

    // L�gica para os bot�es de scroll da lista de produtos
    ...
    if (barcodeBtn) {
        barcodeBtn.addEventListener('click', (e) => {
            e.preventDefault();
            // Evitar que algum input permane�a focado
            try {
                if (document.activeElement && document.activeElement !== document.body) {
                    document.activeElement.blur();
                }
            } catch(_) {}
            // Usar implementa��o central
    ...
    const applyScannedBarcode = (code) => {
        ...
        if (barcodeSearchBar) {
            ...
            // Tenta abrir automaticamente a compara��o do produto lido
            try {
                const products = getFromLocalStorage('products');
                ...
                if (prod && typeof openCompareModal === 'function') {
                    ...
                    try { showToast(`C�digo lido: ${code}. Abrindo compara��o�`, { type: 'success' }); } catch (_) {}
                } else {
                    try { showToast(`C�digo lido: ${String(code)} � produto n�o encontrado`, { type: 'info' }); } catch (_) {}
                }
            } catch(_) {}
            ...
            try { showToast(`C�digo lido: ${String(code).slice(0, 18)} � resultados filtrados`, { type: 'success' }); } catch (_) {}
        }
    };
    ...
    const stopScanner = () => {
        ...
        // opcional: restaurar sele��o de dispositivos (manter atual)
    ...
    document.addEventListener('keydown', (e) => {
        ...
        // Impedir teclas de navega��o (setas, page up/down, home, end)
    ...
    // Tornar as fun��es dispon�veis globalmente
    ...
    window.addEventListener('productsLoaded', (event) => {
        ...
        if (typeof window.renderProducts === 'function') {
            window.renderProducts();
            console.log('? Produtos do Firebase renderizados ap�s evento');
...
// Fun��o global para abrir detalhes (mantida fora do listener para acesso geral)
...
window.addEventListener('load', () => {
    try {
        ...
        console.warn('Falha no fallback de renderiza��o:', e);
    }
...
window.addEventListener('productsLoaded', (event) => {
    ...
    // COMENTADO: // Fechar modais com anima��o
    ...
    // COMENTADO: }
    // COMENTADO: } catch (err) {
    // COMENTADO: // seguran�a contra ambientes inesperados
    ...
    // Handler duplicado (segunda se��o) substitu�do: abrir scanner sem focar input
    const barcodeBtn = document.getElementById('barcode-search-btn');
    if (barcodeBtn) {
        barcodeBtn.addEventListener('click', (e) => {
            e.preventDefault();
